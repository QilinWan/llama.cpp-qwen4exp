...
void llama_model_qwen4exp::load_arch_tensors(llama_model_loader & ml) {
    ...
    // a draft-only export carries neither these nor the trunk, hence trunk_flags
    ...
    for (int il = 0; il < (int) hparams.n_layer_all; ++il) {
        ...
        layer.nextn.hnorm   = create_tensor(tn(LLM_TENSOR_NEXTN_HNORM,   "weight", il), { n_embd, hc }, TENSOR_ALLOW_RESHAPE | flags);
...
llama_model_qwen4exp::graph_mtp::graph_mtp(const llama_model & model, const llm_graph_params & params) :
    ...
    // grouped RMSNorm over the wide stream, gamma [n_embd, hc] like the trunk's
