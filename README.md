...
- **内存不是必须这么大，有两条已验证的压缩路径**：换更小的量化档可以显著降低占用；换更大显存的 GPU 并把更多专家层交给 GPU（`--n-cpu-moe`）也能降低系统内存占用——后者我们在 Ornith-1.5-35B 上做过验证。上面的数字只是我们这一套"16GB 显存 + 全专家 CPU"组合的实测值，不是该架构的硬性要求。
