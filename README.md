...
运行权重（也是我们唯一常跑的一款）：**Qwen3.8-Flash-Next（qwen4exp 架构）Q4_K_XL 量化**，约 177B 总参数的 MoE（512 专家、激活 10 个），权重约 111GB，专家常驻 CPU（`--cpu-moe`），配一个约 2GB 的 MTP draft 侧车权重和 Q8_0 的多模态投影，上下文 262144，KV 缓存 q4_0 量化，单并发。
...
- **吃内存**：该权重把专家放在 CPU，需要约 250GB 级系统内存 + 16GB 级显存才能按上述配置运行；小机器不适用。
